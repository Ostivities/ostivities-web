...
const TicketsSelection = () => {
  ...
  // State to manage selected ticket counts, using ticketId as the key
  ...
  // Function to handle ticket increment by ticketId
  const handleIncrement = (index: number) => {
    ...
    if (index >= 0 && index < ticketData.length) {
      ...
      setTicketDetails((prevDetails) => {
        ...
        // If the ticket exists, update its quantity and price
          ...
          updatedDetails[existingTicketIndex] = {
            ...
            ticketPrice:
              ticket.ticketEntity === TICKET_ENTITY.SINGLE
                ? ticket.ticketPrice *
                  (updatedDetails[existingTicketIndex].ticketNumber + 1)
                : ticket.groupPrice *
                    (updatedDetails[existingTicketIndex].ticketNumber + 1) || 0,
            ticketNumber: updatedDetails[existingTicketIndex].ticketNumber + 1,
          };
        } else {
  ...
  // Function to handle ticket decrement by ticketId
  const handleDecrement = (index: number) => {
    ...
    if (index >= 0 && index < ticketData.length) {
      ...
      setTicketDetails((prevDetails) => {
        // Find the existing ticket in the details
        const existingTicketIndex = prevDetails.findIndex(
          (item) => item.ticketName === ticket.ticketName
        );
        ...
        // If the ticket exists and its quantity is greater than 1, decrement its quantity
        if (existingTicketIndex > -1) {
          const existingTicket = updatedDetails[existingTicketIndex];

          if (existingTicket.ticketNumber > 1) {
            updatedDetails[existingTicketIndex] = {
              ...existingTicket,
              ticketPrice:
                ticket.ticketEntity === TICKET_ENTITY.SINGLE
                  ? ticket.ticketPrice * (existingTicket.ticketNumber - 1)
                  : ticket.groupPrice * (existingTicket.ticketNumber - 1) || 0,
              ticketNumber: existingTicket.ticketNumber - 1,
              subTotal: existingTicket.ticketNumber - 1,
            };
          } else {
            // If the ticket quantity is 1, remove it from the details
            updatedDetails.splice(existingTicketIndex, 1);
          }
        }
  ...
  // Calculate the total price and fee whenever selected tickets change
  useEffect(() => {
    const total = Object.entries(selectedTickets).reduce(
      (acc, [ticketId, ticketCount]) =>
        acc +
        ticketCount *
          (ticketData.find((ticket: any) => ticket.id === ticketId)
            ?.ticketPrice || 0),
      0
    );
    const totalFee = Object.entries(selectedTickets).reduce(
      (acc, [ticketId, ticketCount]) =>
        acc +
        ticketCount *
          (ticketData.find((ticket: ITicketDetails) => ticket.id === ticketId)
            ?.ticketFee || 0),
      0
    );
    setTotalPrice(total);
